...
const Mystery = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://www.googleapis.com/books/v1/volumes?q=mystery&startIndex=${startIndex}&maxResults=${totalBooksPerPage}&key=${apiKey}`;
    ...
    className="fiction-section w-[75%] mx-auto lg:flex-row justify-center gap-7 md:flex-row sm:flex-col items-center lg:mt-[150px] sm:mt-[150px] md:mt-[200px] custom-media-query">
      <div className=" flex   rounded-full items-center justify-center mb-5 flex-wrap bg-white ">
        <h2 className=" font-bold text-3xl   text-[#A04AC4] py-5 ">
          <TypewriterText
            texts={[
              "Embark on a Journey of Intrigue and Suspicion",
"Dive into Mysterious Worlds Full of Clues and Riddles",
"Explore the Depths of Puzzling Whodunits",
"Solve Enigmatic Crimes Alongside Clever Detectives",
"Navigate Through Twisty Plots and Hidden Truths",
"Delve into the Unknown as Secrets Unfold",
            ]}
            staticText={"Unravel the Secrets of Enigmatic Mysteries"}
          ></TypewriterText>
        </h2>
      </div>
     {
