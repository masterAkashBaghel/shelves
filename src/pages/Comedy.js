...
const Comedy = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://www.googleapis.com/books/v1/volumes?q=comedy&startIndex=${startIndex}&maxResults=${totalBooksPerPage}&key=${apiKey}`;
      ...
      <div className=" flex   rounded-full items-center justify-center mb-5 flex-wrap bg-white ">
        <h2 className=" font-bold text-3xl   text-[#A04AC4] py-5 ">
          <TypewriterText
            texts={[
              "Get Ready to ROFL",
"Discover Humorous Gems",
"Unwind with Witty Stories",
            ]}
            staticText={ "Laugh Out Loud with Our Comedy Collection"}
          ></TypewriterText>
        </h2>
      </div>
     {
