...
const Searchbar = () => {
 ...
 const{query} = useParams();
  ...
  useEffect(() => {
    // Fetch search results using the Google Books API
    const fetchSearchResults = async () => {
      ...
      try {
        ...
        const apiUrl = `https://www.googleapis.com/books/v1/volumes?q=${query}&startIndex=${startIndex}&maxResults=${totalBooksPerPage}&key=${apiKey}`;
  ...
  if (!query || fictionBooks.length === 0) {
    return <div className="text-center mt-5">Search not found.</div>;
  }
    ...
    className="fiction-section w-[75%] mx-auto lg:flex-row justify-center gap-7 md:flex-row sm:flex-col items-center lg:mt-[150px] sm:mt-[150px] md:mt-[200px] custom-media-query">
      <div className=" flex   rounded-full items-center justify-center mb-5 flex-wrap bg-white ">
        <h2 className=" font-bold text-3xl   text-[#A04AC4] py-5 ">
          <TypewriterText
                    texts={['Search for Your Favorite Books', 'Discover New Reads', 'Explore Vast Collection']}

            staticText={"Search for books categories authors "}
          ></TypewriterText>
        </h2>
      </div>
     {
