...
const Drama = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://www.googleapis.com/books/v1/volumes?q=drama&startIndex=${startIndex}&maxResults=${totalBooksPerPage}&key=${apiKey}`;
    ...
    className="fiction-section w-[75%] mx-auto lg:flex-row justify-center gap-7 md:flex-row sm:flex-col items-center lg:mt-[150px] sm:mt-[150px] md:mt-[200px] custom-media-query">
      <div className=" flex   rounded-full items-center justify-center mb-5 flex-wrap bg-white ">
        <h2 className=" font-bold text-3xl   text-[#A04AC4] py-5 ">
          <TypewriterText
            texts={[
                "Explore the Intrigue and Emotion of Dramatic Stories",
                "Unveil Unforgettable Characters and Twists",
                "Immerse Yourself in the Drama of Human Relationships",
                "Experience the Tension and Suspense of Gripping Plots",
                "Discover the Power of Words to Move Hearts",
            ]}
            staticText={"Dive into the World of Captivating Drama"}
          ></TypewriterText>
        </h2>
      </div>
     {
