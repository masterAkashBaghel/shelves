...
const Romance = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://www.googleapis.com/books/v1/volumes?q=romance&startIndex=${startIndex}&maxResults=${totalBooksPerPage}&key=${apiKey}`;
    ...
    className="fiction-section w-[75%] mx-auto lg:flex-row justify-center gap-7 md:flex-row sm:flex-col items-center lg:mt-[150px] sm:mt-[150px] md:mt-[200px] custom-media-query">
      <div className=" flex   rounded-full items-center justify-center mb-5 flex-wrap bg-white ">
        <h2 className=" font-bold text-3xl   text-[#A04AC4] py-5 ">
          <TypewriterText
            texts={[
              "Discover Heartfelt Tales of Love and Passion",
              "Indulge in Stories That Ignite the Heart",
              "Dive into Exciting Imaginations",
              "Experience the Emotions of Love's Journey",
              "Unwind with Captivating Stories of Love",
            ]}
            staticText={"Embrace the Allure of Romance"}
          ></TypewriterText>
        </h2>
      </div>
     {
